...
Dialog {
    ...
    title: creatingNew ? qsTr("Add Pillar") : qsTr("Edit Pillar")
    ...
    function openForPillar(nextPillarId) {
        const pillar = appController.pillarDetails(nextPillarId)
        if (!pillar.pillarId) {
            saveError = qsTr("Pillar not found.")
            return
        }

        creatingNew = false
        pillarId = pillar.pillarId
        originalKey = pillar.key
        keyField.text = pillar.key
        displayNameField.text = pillar.displayName
        activeBox.checked = pillar.isActive
        keyError = ""
        saveError = ""
        open()
    }
    ...
    function refreshKeyState() {
        ...
        const validation = appController.validatePillarKey(pillarId, trimmedKey)
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            TextField {
                ...
                placeholderText: qsTr("pillar_key")
        ...
        DialogButtonBox {
            ...
            onAccepted: {
                ...
                root.saveError = ""
                if (appController.savePillar(root.pillarId,
                                             keyField.text,
                                             displayNameField.text,
                                             activeBox.checked)) {
                    root.close()
