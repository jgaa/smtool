...
std::vector<Domain::CalendarEntry> DashboardRepository::calendarEntries(bool includeArchived, bool includePublished, const QString &searchQuery) const
{
    ...
    while (query.next()) {
        ...
        if (!std::ranges::all_of(parsed.tagTerms, [&](const auto &tag) { return tagsCache.split(u' ', Qt::SkipEmptyParts).contains(tag); })) {
            continue;
        }
